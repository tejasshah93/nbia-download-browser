...
/* global jnlpURL, jnlpArgument, jnlpDownloadServerUrl, jnlpUserId, jnlpPassword, jnlpIncludeAnnotation */ // eslint-disable-line no-unused-vars
  ...
  asyncLoop({
    ...
    functionToLoop: function (loop, i) {
      ...
      manifest_i[3], Math.round(
          ((parseInt(manifest_i[6], 10) + parseInt(manifest_i[7], 10) * 1.0) /
...
var downloadManifestSchema = function (cbDownloadManifestSchema) {
  var x = new XMLHttpRequest(); // eslint-disable-line no-undef
...
var bootJnlp = function (messageJnlpURL) {
  bundle.fetchJnlp(launchData, messageJnlpURL, function (errFetchJnlp, fetchJnlpURL) {
    if (errFetchJnlp) {
      ...
      jnlpURL = fetchJnlpURL; // eslint-disable-line
      var x = new XMLHttpRequest(); // eslint-disable-line no-undef
      ...
      x.onreadystatechange = function () {
        if (x.readyState === 4 && x.status === 200) {
          ...
          if (window.DOMParser) {
            ...
            alert('Error: window.DOMParser not supported'); // eslint-disable-line no-undef
          }
          jnlpArgument = xmlDoc.getElementsByTagName('argument')[0].childNodes[0].nodeValue; // eslint-disable-line
          var properties = xmlDoc.getElementsByTagName('property');
          for (var i = 0; i < properties.length; i++) {
            if (properties[i].getAttribute('name') === 'jnlp.includeAnnotation') {
              jnlpIncludeAnnotation = true; // eslint-disable-line
            } else if (properties[i].getAttribute('name') === 'jnlp.userId') {
              jnlpUserId = properties[i].getAttribute('value'); // eslint-disable-line
            } else if (properties[i].getAttribute('name') === 'jnlp.password') {
              jnlpPassword = properties[i].getAttribute('value'); // eslint-disable-line
            } else if (properties[i].getAttribute('name') === 'jnlp.downloadServerUrl') {
              jnlpDownloadServerUrl = properties[i].getAttribute('value'); // eslint-disable-line
            }
          }
...
$(document).ready(function () {
  ...
  dTable = $('#displaySchema').DataTable({ // eslint-disable-line
    'order': [[ 7, 'asc' ]],
    'select': true,
    'scrollY': '400px',
    'scrollCollapse': true,
